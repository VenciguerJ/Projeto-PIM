...
void calculadora_price(float principal, int numParcelas, int nroimovel) {
...
void aluguel(char *arquivo,float valorimovel,char cpf[12], int nroimovel){//Mostra os valores de aluguel
	...
	int valuguel=1;
	int i;
	int nroproposta = qtd_propostas(arquivo);
	...
	do{	
		...
		i = 1;
		if(resp=='S'||resp=='s'){
			fprintf(arquivo, "%s %i %i %i %i\n", cpf_user, nroproposta, valuguel, nroimovel,status);
	...
	do{	printf("1 - Pagamento à vista\n2 - Financiamento\n\nEscolha:");
		...
		if(resp3==1){ // pagamento à vista
			...
			do{ 
				...
				scanf("%c", &resp4);
				if(resp4=='S'||resp4=='s'){
					fprintf(arquivo, "%s %i %i %i %i\n", cpf_user, nroproposta, compra,nroimovel,status);
					printf("Proposta Enviada com sucesso! Iremos entrar em contato com voce para darmos continuidade no processo");
					i = 0;
				}
					...
					do{	printf("Deseja verificar as possibilidades de Financimento ou Aluguel? 1 - Aluguel\n2 - Financiamento\n3 - Sair\n\nEscolha:");
						...
						if(resp3==1){
							...
							scanf("%i",&nroparcelas);
							calculadora_price(valorimovel,nroparcelas,nroimovel);
							i = 0;
						}
						else if(resp3==3){
							i = 0;
...
void nova_proposta(float valorimovel,int nroimovel){
	...
	do{	if(resp==1){ // aluguel
			aluguel(fpropostas,valorimovel,cpf_user,nroimovel);
			i = 0;
			}
		
		else if(resp==2){ // compra
			compra(fpropostas,valorimovel,cpf_user,nroimovel);
			i = 0;		
		}
